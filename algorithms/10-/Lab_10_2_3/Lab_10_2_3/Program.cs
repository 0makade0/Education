...
namespace Lab_10_2_3
{
    internal class Program
    {
        static StreamReader In1 = new StreamReader("input2.txt");
        static StreamWriter Out1 = new StreamWriter("output2.txt");
        ...
        static bool Route(char[,] labyrinth)
        {
            ...
            while (queue.Count != 0)
            {
                ...
                if (j > 0 && labyrinth[j - 1, i] == 'E')
                    return end = true;

                if (j < R - 1 && labyrinth[j + 1, i] == 'E')
                    return end = true;

                if (i > 0 && labyrinth[j, i - 1] == 'E')
                    return end = true;

                if (i < C - 1 && labyrinth[j, i + 1] == 'E')
                    return end = true;

                if (j > 0 && labyrinth[j - 1, i] == '.')
                {
                    queue.Enqueue(j - 1);
                    queue.Enqueue(i);
                    labyrinth[j - 1, i] = 'X';
        ...
        static void Main(string[] args)
        {
            ...
            {
                "", "R", "G", "B", "Y", "R G", "R B", "R Y", "G B", "G Y", "B Y", "R G B", "R G Y", "R B Y", "G B Y",
                "R G B Y"
            }; 
